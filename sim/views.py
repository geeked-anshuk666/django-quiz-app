...
def get_questions(request, is_start=False) -> HttpResponse:
    selected_topic = request.GET.get('quiz_id')  # Get selected topic from request

    if is_start:
        request = _reset_quiz(request)
        question = _get_first_question(request, selected_topic)
    else:
        question = _get_subsequent_question(request, selected_topic)
        if question is None:
...
def _get_first_question(request, quiz_id=None) -> Question:
    if quiz_id is None:
        quiz_id = request.POST['quiz_id']
    return Question.objects.filter(quiz_id=quiz_id).order_by('id').first()


def _get_subsequent_question(request, quiz_id=None) -> Optional[Question]:
    if quiz_id is None:
        quiz_id = request.POST['quiz_id']

    # Ensure 'question_id' exists in session before accessing it
    if 'question_id' not in request.session:
        request.session['question_id'] = 0  # Initialize to a valid starting value (first question)
    ...
    except Question.DoesNotExist:  # No more questions for this quiz
        return None
